...
Run wallets on a machine you control, with a strong master key and unique `AGENT_WALLET_DIR`. This is **not** a custody-policy product: when enabled and funded, the agent can spend what you deposited. There are no spend caps, allowlists, or confirm gates.
...
| Spend caps / allowlists | **none** | Removed. Funding authorizes spend. |
...
A **tiny-value** tx (e.g. 0.01 PLS transfer) can still need **substantial PLS for gas**. Funding pays gas and authorizes spend.
...
| Approve / token transfer | ~200–500 PLS | Operator-trust: funding authorizes |
...
1. **Create** — `create_agent_wallet` (optional `label`). Returns public address + wallet id only.
...
3. **Fund later (gas-aware)** — send enough PLS for **gas + intended value** (see table above); not “1 PLS dust” Ethereum thinking. Create/verify does not require funding. Funding is authorization.
4. **Then signing** — prefer **native transfer first**, then approve/token, then swap. Flow: `inspect_tx_intent` (if calldata unclear) → `propose_agent_tx` → read **`reviewSummary` / `agentGuidance` / `safetyHints`** (value vs gas) → `execute_agent_tx`.

Do **not** fund until you accept that the agent can spend that balance. On any concern: `kill_switch`.
...
The design goal: **keys stay encrypted and never appear in tool output**. Send-time blocks are kill/disabled/invalid only. Funding a wallet is authorization to spend it.
...
Write tool descriptions include a standard `WRITE_TOOL_WARNING` (see `src/utils/safety.ts` and `src/tools/wallet/index.ts`). Wallet writes run when wallets are on — no `confirm=true` or MRTR gate.

**Operator-trust:** this product is **not** a custody-policy / spend-limit backstop. **Funding the agent is authorization.** There are no spend caps, allowlists, token-notional denies, or confirm gates. Real operator controls:
...
4. `kill_switch` / `enabled=false` for emergencies  
5. Unique `AGENT_WALLET_DIR` per process (multiproc)  

**Practical flow:** **inspect → propose → review `reviewSummary` (destination, value vs gas) → execute**.

### Reading review summaries (v0.1.15+, intelligence v0.1.16)

Safe pattern:

1. Optional **`inspect_tx_intent`** for calldata meaning without a wallet  
2. **`propose_agent_tx`** (or `agent_wallet_check_policy` for a kill/disabled dry-run)  
3. **Read `reviewSummary` + `policyCheck` + `agentGuidance`**  
4. **`execute_agent_tx`**
...
| `decision` | `allow` \| `deny` |
| `agentGuidance` | `ready` \| `blocked` (kill/disabled/invalid only) |
...
| `tokenNotional` | Pattern, confidence (decode only — never a send gate) |
| `checksApplied` | kill_switch / enabled / valid_input |
...
**How to use it:** If `decision` is `deny` or `agentGuidance` is `blocked`, do **not** execute (kill/disabled/invalid). `inspect_tx_intent` reports `decodeComplete` for visibility only — it does not refuse sends.
...
**This is not a custody-policy product.** If wallets are enabled and funded, ordinary native transfers and contract calls are **not** blocked by allowlists, PLS caps, or token-notional rules. Older on-disk records may still contain unused cap/allowlist fields — those are ignored.
...
- **`reviewSummary`** adds `agentGuidance` (`ready` \| `blocked`), `decodeKnowledge` (known vs unknown vs truncated), `movementExplanations`, and `safetyHints` (includes PulseChain EIP-1559 / BEATS gas reality, value vs gas vs in-wallet total, prefer native then approve/swap).
- **`inspect_tx_intent`** tool: pure local decode (no wallet, no chain I/O) for agents to inspect calldata before propose/execute. Reports `decodeComplete` and residual uncertainty honestly (not full simulation; gas cost in PLS not estimated here). Decode quality never blocks send.
...
**Chosen rule:** when the outer transaction has `valueWei > 0`, token-notional attributes the **full outer native value once** as a single `native` movement (`role: nativeValue`). Zero-value outer multicalls are unchanged (token inners only).
...
**Practical guidance:** fund only what you accept the agent may spend; use kill_switch in emergencies.
...
- Double-broadcast of the same proposal (or parallel spends)
...
**Interrupted settlement recovery:** if the process dies after the barrier but before `executed`, call **`settle_interrupted_broadcast`**. It never re-broadcasts; it finishes local spend (idempotent) and marks `executed`. Requires an existing `txHash` on the proposal.
...
5. On `execute_agent_tx` / `sign_and_send` / `transfer_pls`, re-check kill/enabled, then sign and broadcast.
...
## MRTR secret (leftover; unused for wallet writes)
...
4. **Operator-trust:** funding the agent is authorization. There are no spend caps or allowlists.
...
8. On incident: `kill_switch` immediately.
9. Never paste private keys or master keys into the LLM.
10. `AGENT_WALLET_MRTR_SECRET` is unused for wallet writes; HTTP + wallets does not require it. If set, it must be ≥32 bytes UTF-8.
