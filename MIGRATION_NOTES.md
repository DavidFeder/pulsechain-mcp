...
> **Current package version on `main`:** see `package.json` (**1.0.4**). Dual-era MCP remains the protocol default. The TypeScript MCP SDK is pinned to stable **`2.0.0`** (released for the final **2026-07-28** spec). No product-facing `MAX_PLS_*` spend-cap knobs in examples/templates. Agent install default is research-only; wallets-on uses launcher + `.env.wallet` (no master key in host config). Config/env recovery text points at write-only key generation (no console.log key recipe).
